...
window.onload = function () {
    ...
    var currentLoanTerm = document.getElementById("LoanTerm"); //get drop down select for term
    ...
    function filterLoans() {
        if (loanAmount.value == "" && currentLoanTerm.value == "") { // filter by currency
        ...
        if (loanAmount.value != "" && currentSelect == "Any Currency" && currentLoanTerm.value == "") { //filter by amount
            myCurrency.forEach(element => {
        ...
        if (loanAmount.value != "" && currentSelect != "Any Currency" && currentLoanTerm.value == "") { //filter by amount and currency
            myCurrency.forEach(element => {
        ...
        if (loanAmount.value == "" && currentSelect == "Any Currency") { // filter by term
            myCurrency.forEach((element) => {
                ...
                if (parseInt(currentLoanTerm.value) > termValue - 3 && parseInt(currentLoanTerm.value) < termValue + 3) { //chech if chosen value and deposite currency is the same
                    element.style.display = " block"; // show element if value and currency are the same
                } else {
                    element.style.display = "none"; // hide if are not the same
                }

                if (currentLoanTerm.value == "") { //if chosen value = all , show all desposites
                    element.style.display = " block";
                }
            });
        }

        if (loanAmount.value != "" && currentSelect == "Any Currency" && currentLoanTerm.value != "") { //filter by term and value
            myCurrency.forEach((element) => {
                var termValue = parseInt(element.getElementsByClassName("max-term")[0].innerHTML);

                if (parseInt(currentLoanTerm.value) > termValue - 3 && parseInt(currentLoanTerm.value) < termValue + 3 && element.getElementsByClassName("loan-amount")[0].innerHTML >= parseInt(loanAmount.value)) { //chech if chosen value and deposite currency is the same
                    element.style.display = " block"; // show element if value and currency are the same
                } else {
                    element.style.display = "none"; // hide if are not the same
                }
        ...
        if (loanAmount.value == "" && currentSelect != "Any Currency" && currentLoanTerm.value != "") {//filter by term and currency
            myCurrency.forEach((element) => {
                var termValue = parseInt(element.getElementsByClassName("max-term")[0].innerHTML);

                if (parseInt(currentLoanTerm.value) > termValue - 3
                    && parseInt(currentLoanTerm.value) < termValue + 3
        ...
        if (loanAmount.value != "" && currentSelect != "Any Currency" && currentLoanTerm.value != "") { //filter by term, value and currency
            myCurrency.forEach((element) => {
                var termValue = parseInt(element.getElementsByClassName("max-term")[0].innerHTML);
    ...
    filterReset.onclick = function () {
        ...
        currentLoanTerm.value = ""; //set drop-down terms to any term
