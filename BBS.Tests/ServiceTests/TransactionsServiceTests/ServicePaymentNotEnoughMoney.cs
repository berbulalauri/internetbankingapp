...
{
	[TestClass]
	public class ServicePaymentNotEnoughMoney : Specification
	{
		...
		{
			expected = new Models.CustomExceptions.NotEnoughMoneyException();
			inputUser = new ClaimsPrincipal();
			inputLogger = new Mock<ILogger>();
			inputTransaction = new Transactions { Service = new Service() { Id = 1, FixedFee = 2.0m }, Amount = 6m };
		...
		{
			unitOfWork.Setup(x => x.CardRepository.GetCardIncludesAsync(It.IsAny<int>())).ReturnsAsync(new Card { Id = 1, UserId = 2, Account = new Account {Balance=5 } });
