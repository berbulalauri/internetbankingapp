...
{
    public sealed class TermMaximumAndMinimumValidationAttribute : ValidationAttribute
    {
        private string _minimum;
        private string _minimumDisplayName;
        private const string _errorMessage = "'{0}' must be greater than '{1}'";

        public TermMaximumAndMinimumValidationAttribute(string minimum)
            : base(_errorMessage)
        {
            _minimum = minimum;
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (minimum != null && value != null && (int)value < (int)minimumValue)
            {
